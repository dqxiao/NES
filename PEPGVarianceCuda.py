...
class PEPGVarianceCuda:
    ...
    def tell(self, reward_table_result):
        ...
        # calculate the reward squaare derivates and Value 
        reward_s= torch.pow(reward,2) 
        stdev_reward_s = reward_s.std()
        reward_s_avg = (reward_s[:self.batch_size] + reward_s[self.batch_size:]) / 2.0
        rsS = reward_s_avg - b*b
        rsS = rsS.view(1,self.batch_size)
        rs_sigma = torch.mm(rsS,S) /(2*self.batch_size* stdev_reward_s) 
        #done 
        
        rsT = (reward_s[:self.batch_size] - reward_s[self.batch_size:])
        rsT = rsT.view(1,self.batch_size)
        
        delta_mu  = self.learning_rate*torch.mm(rsT,epsilon)
        delta_mu  = delta_mu - 2*b*change_mu
        delta_mu *= self.diversity_base 
        
        
        delta_s_sigma = self.sigma_alpha * rs_sigma 
        delta_s_sigma -=2*b*delta_sigma
        delta_s_sigma *= self.diversity_base
        # update the function 
        ...
        # calculate the rewards derviate 
        
        
        self.sigma = self.sigma + delta_s_sigma 
